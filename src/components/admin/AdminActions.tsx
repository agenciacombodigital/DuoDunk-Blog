...
import { Bot, Calendar, Edit3, RefreshCw, Trash2, AlertTriangle } from 'lucide-react';
...
interface AdminActionsProps {
  ...
  onProcess: () => void;
  onDeleteAll: () => void;
  onRetryRateLimited: () => void; // Novo
}

export default function AdminActions({ isLoading, onScrape, onProcess, onDeleteAll, onRetryRateLimited }: AdminActionsProps) {
  return (
    <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <button onClick={onRetryRateLimited} disabled={isLoading} className="bg-red-600 hover:bg-red-700 text-white font-bold px-8 py-3 rounded-xl transition-all duration-300 transform hover:scale-105 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><AlertTriangle className="w-5 h-5" /> Retentar Rate Limit</button>
