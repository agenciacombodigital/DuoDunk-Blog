...
export default function AdminPage() {
  ...
  const deletePublishedArticle = async (articleId: string) => {
    if (!window.confirm('Tem certeza que deseja deletar esta notícia publicada? A ação é irreversível.')) return;
    const toastId = toast.loading("Deletando notícia...");
    try {
      const { error } = await supabase.from('articles').delete().eq('id', articleId);
      if (error) throw error;
      toast.success("Notícia deletada com sucesso.", { id: toastId });
      await loadPublished();
    } catch (error: any) {
      toast.error("Erro ao deletar.", { id: toastId, description: error.message });
    }
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="h-16 w-16 animate-spin text-primary" /></div>;
      ...
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        {pendingApproval.length > 0 && (
          <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
            ...
            <div className="space-y-6">{pendingApproval.map((article) => (<div key={article.id} className="bg-gray-800 rounded-xl overflow-hidden border border-gray-700"><div className="p-6"><div className="mb-4 p-4 bg-gray-900/50 rounded-lg border border-gray-700"><label className="block text-sm text-gray-400 mb-2 font-semibold">📸 Imagem do Artigo:</label>{article.image_url && <img src={article.image_url} alt="Preview" className="w-full h-48 object-cover rounded-lg mb-3" />}<input type="file" accept="image/*" onChange={(e) => { const file = e.target.files?.[0]; if (file) handleImageUpload(article.id, file); }} disabled={uploadingImage === article.id} className="block w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-cyan-600 file:text-white hover:file:bg-cyan-700 file:cursor-pointer cursor-pointer" />{uploadingImage === article.id && <p className="text-xs text-cyan-400 mt-2 animate-pulse flex items-center gap-2"><Loader2 className="w-3 h-3 animate-spin" />Fazendo upload...</p>}</div><h3 className="text-xl font-bold text-white mb-2">{article.title}</h3><p className="text-gray-400 text-sm mb-4 leading-relaxed">{article.summary}</p><details className="mb-4"><summary className="cursor-pointer text-secondary hover:text-cyan-300 text-sm font-semibold">📄 Ver conteúdo completo</summary><div className="mt-4 prose prose-invert prose-sm max-w-none bg-black p-4 rounded-lg overflow-auto max-h-96" dangerouslySetInnerHTML={{ __html: article.body }} /></details>{article.tags && <div className="flex flex-wrap gap-2 mb-4">{article.tags.map((tag: string) => (<span key={tag} className="px-2 py-1 bg-gray-700 text-gray-300 text-xs rounded">#{tag}</span>))}</div>}<div className="flex gap-3"><button onClick={() => approveArticle(article.id)} className="btn-success flex-1 flex items-center justify-center gap-2"><CheckCircle className="w-5 h-5" />Aprovar e Publicar</button><button onClick={() => rejectArticle(article.id)} className="btn-danger flex items-center justify-center gap-2"><Trash2 className="w-5 h-5" />Rejeitar</button></div></div></div>))}</div>
          </div>
        )}
        ...
        {published.length > 0 && (
          <div className="bg-gray-900 rounded-xl p-6 border border-gray-800">
            ...
            <div className="space-y-2">{published.map((article) => (<div key={article.id} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg border border-gray-700 hover:border-green-700/50 transition-colors"><div className="flex-1 min-w-0"><a href={`/artigos/${article.slug}`} target="_blank" rel="noopener noreferrer" className="text-white font-semibold text-sm hover:text-green-400 truncate block">{article.title}</a><div className="flex items-center gap-3 mt-1"><span className="text-xs text-gray-500">{article.source}</span><span className="text-xs text-gray-500">{new Date(article.published_at).toLocaleDateString('pt-BR')}</span><span className="text-xs text-cyan-400">{article.views || 0} views</span></div></div><div className="flex items-center gap-2 ml-4 flex-shrink-0"><Link to={`/admin/editar/${article.slug}`} className="px-3 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-colors text-sm font-semibold flex items-center gap-1.5"><Edit className="w-4 h-4" /> Editar</Link><button onClick={() => deletePublishedArticle(article.id)} className="px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors text-sm font-semibold flex items-center gap-1.5"><Trash2 className="w-4 h-4" /> Deletar</button></div></div>))}</div>
          </div>
        )}
      </div>
    </div>
