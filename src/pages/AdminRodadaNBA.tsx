...
export default function AdminRodadaNBA() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dateObj = new Date(`${date}T12:00:00Z`); // Use UTC noon to avoid timezone issues
      ...
      body += `<h2 style="color: #FA007D; font-size: 28px; margin-bottom: 20px;">🏀 JOGOS DE HOJE (${dateStr})</h2>`;
      body += '<div style="border-bottom: 3px solid #FA007D; margin: 20px 0;"></div>';
      ...
      games.forEach((game, index) => {
        ...
        body += `
          <div style="margin-bottom: 25px; padding: 20px; background: ${isMainGame ? '#FFF1F8' : '#f9f9f9'}; border-radius: 12px; border: 2px solid ${isMainGame ? '#FA007D' : '#e1e1e1'};">
            ${isMainGame ? '<h3 style="color: #FA007D; margin: 0 0 15px 0; font-size: 18px;">🔥 JOGO PRINCIPAL</h3>' : ''}
            
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px; flex-wrap: wrap;">
              <span style="font-size: 18px; font-weight: bold; color: #333;">${game.time} (BR)</span>
              <span style="padding: 4px 12px; background: ${isMainGame ? '#FA007D' : '#333'}; color: white; border-radius: 20px; font-size: 12px; font-weight: 600;">
      ...
      body += '</ul>';
      body += '<div style="margin-top: 30px; padding: 15px; background: #FFF1F8; border-left: 4px solid #FA007D; border-radius: 4px;">';
      ...
      const { error } = await supabase.from('articles').insert({
        ...
        image_url: 'https://images.unsplash.com/photo-1546519638-68e109498ffc?w=1200&h=630&fit=crop&q=80',
          ...
          <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
            ...
            <div className="bg-gray-900 rounded-lg p-4 border border-gray-700">
              ...
              <p className="text-gray-400 text-sm">
                {games[0]?.awayTeam || 'Time 1'} x {games[0]?.homeTeam || 'Time 2'}{games.length > 1 ? ` e mais ${games.length - 1} confrontos...` : ''}
              </p>
            </div>
          </div>
